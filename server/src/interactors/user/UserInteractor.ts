...
import {
  ...
  IUpdateUser,
  IUserPoint,
} from "../../entities/interfaces/UserInterfaces";
...
export class UserInteractor implements IUserInteractor {
  ...
  async increaseUserPoint(input: IUserPoint) {
    ...
    return await this.repository.update(input.userId, {
      totalPoints: user.totalPoints + input.point,
    });
  }
  async decreaseUserPoint(input: IUserPoint) {
    ...
    if (user.totalPoints < input.point)
      throw new BadRequestsException(
        "Not enough points.",
  ...
  async updateUser(input: IUpdateUser) {
    var data: any = { ...input };
    delete data.id;
    if ((await this.repository.findUnique({ id: input.id })) == null)
      throw new BadRequestsException(
  ...
  async changeProfilePicture(input: IChangeProfilePicture) {
    ...
    var newAddres = await this.imageService.uploadSingleImage(
      input.profilePicture,
      "users/" + user.username + "/profilePicture/"
    );
    this.imageService.delete(oldAddress);
    return await this.repository.update(input.id, {
      profilePictureAddress: newAddres,
    });
  }
