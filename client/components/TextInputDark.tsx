...
const TextInputDark = ({ label, placeholder, value, setValue, width, type }: { label: ReactNode, placeholder: string, value: string, setValue: any, width: DimensionValue, type?: KeyboardTypeOptions }) => {
    return (
        <View style={{ width: width }}>
            <Text style={{ color: "rgba(255,255,255,1)", marginBottom: 10 }}>{label}</Text>
            <TextInput
                style={{
                    color: "white", paddingLeft: 20, fontSize: 18, width: "100%", height: 60, borderRadius: 10, backgroundColor: "rgba(0, 0, 0, 0.21)"
                }}
                value={value}
